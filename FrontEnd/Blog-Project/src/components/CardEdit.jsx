...
const CardEdit = ({ CardObj }) => {
  const [localCardObj, setLocalCardObj] = useState(CardObj);

  const navigate = useNavigate();

  useEffect(() => {
    // Update localCardObj when CardObj changes
    setLocalCardObj(CardObj);
  }, [CardObj]);
  ...
  return (
    <>
      <div className="relative flex flex-col items-center justify-evenly w-96 rounded-2 m-4 border-solid border-1 p-4 shadow-md hover:shadow-lg">
        ...
        <Link to={{
            pathname: `/my-blogs/${CardObj.id}`,
            state: { CardObj : localCardObj }
          }} className="absolute top-2 right-12 inline-flex items-center py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-md">
